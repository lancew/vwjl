...
sub get_user_data {
                      ...
                      my @internal_user_data = @_; # This line takes the user_data passed to us from the previous routine (@_) and allocates it to our internal user data array
                      my $entered_id = $internal_user_data[0];  # just so that it is clearer later we create a variable from the passed data
                       ...
                       $dbh->func( 'users', 'CSV', 'data/users_csv', 'ad_catalog'); # Connect to the users_csv data file

                       # select from the datafile the id for the user ID from the array paased from the previous sub routine
                       my @parameters = ($entered_id);                          # Theese are the parameteres we will use in the SQL command above
                       my $sql = "SELECT * FROM users WHERE id = ?";       # this is the SQL command we want to execute
...
sub shiai_count {
                   # This sub rotine gets the Judoka the user has from judoka_csv file and returns the number
                    print p("START of shiai_count") if $DEBUG;
                      ...
                      my @internal_shiai_data = @_; # This line takes the user_data passed to us from the previous routine (@_) and allocates it to our internal user data array
                      my $entered_id = $internal_shiai_data[0];  # just so that it is clearer later we create a variable from the passed data

                       # Use DBI to connect to the users_csv datafile
                       #-----------------------------------------------

                       my $dbh = DBI->connect('dbi:AnyData(RaiseError=>1):'); # tell DBI we want to use the Anydata module in ./MyLibs
                       $dbh->func( 'shiai_db', 'CSV', 'data/shiai_csv', 'ad_catalog'); # Connect to the _csv data file

                       # select from the datafile the id for the user ID from the array paased from the previous sub routine
                       my $sql = "SELECT * FROM shiai_db WHERE owner_id = ?";       # this is the SQL command we want to execute
                       my @params = ($entered_id);                          # Theese are the parameteres we will use in the SQL command above
                       print "$sql\n[@params]\n" if $DEBUG;                 # if we are in debug mode print the SQL statement
                       ...
                       while (@results=$sth->fetchrow_array) {                       # This while loop continues till there is no more reults
...
sub print_shiai_data_form {
    ...
    # The numbers indicate their position in the array(s)
                         ...
                         print p("Shiai Name: ", textfield("shiai_name"), " - This is the name of the tournament");
                         print p("Close Date: ", textfield("closedate"), " - This is date that entries close to the tournament");
...
#                         print p("Shiai Type: ", textfield("type"), " - This is which type of tournament format this tournament is");
                         print p("Shiai Date: ", textfield("eventdate"), " - the date this shiai will begin.");
       	                 print p("Entry Fee: ", textfield("entry_fee"), " - this is how much it will cost players to enter the event");
       	                 print p("First Prize: ", textfield("first_prize"), " - This is how much the winner will receive");
       	                 print p("Second Place Prize: ", textfield("second_prize"), " - This is how much the second place getter will receive");
       	                 print p("Third Place Prize: ", textfield("third_prize"), " - This is how much the third place getter will receive");
       	                 print p("Fourth Place Prize: ", textfield("fourth_prize"), " - This is how much the fourth place getter will receive");
       	                 print p("Fifth Place Prize: ", textfield("fifth_prize"), " - This is how much the fifth place getter will receive");
                         print p("Description: ", textfield("desc"), " - This is a short description of this event");
...
#                                  *     *        *      *                                           *                     *            *           *           *           *           *
                       ...
                       $shiai_data[1] = "BLANK_for_now";    # Now take the parameters from the completed form and add them to the array
...
sub validate_shiai_data {
                      ...
                      print p("Start of validate_shiai_data") if $DEBUG;
                       my @internal_data = @_;                       # Take the array passed to this routine and assign it to an internal array
...
#                                  *     *        *      *                                           *                     *            *           *           *           *           *
 ...
 # user_id is already in the data array at this stage.
                      print p("Start of generate_initial_values") if $DEBUG;
                       my @internal_shiai_data = @_;                       # Take the array passed to this routine and assign it to an internal array
                        $internal_shiai_data[1] = $internal_shiai_data[0] . $internal_shiai_data[2];      # Make the unique ID for this Judoka the users ID plus the Judoka name.
                        $internal_shiai_data[6] = "for now just an abitrary data filed till I pass the users name OK!";
                       ...
                       print p("Validated data + initial values = ", @internal_shiai_data) if $DEBUG;
...
sub add_shiai_to_db {
             ...
             my @internal_shiai_data = @_; # This line takes the user_data passed to us from the previous routine (@_) and allocates it to our internal user data array
             my $entered_shiai_id = $internal_shiai_data[1];  # just so that it is clearer later we create a variable from the passed data

             # Use DBI to connect to the judoka_csv datafile
             #-----------------------------------------------
             my $dbh = DBI->connect('dbi:AnyData(RaiseError=>1):'); # tell DBI we want to use the Anydata module in ./MyLibs
             $dbh->func( 'shiai_db', 'CSV', 'data/shiai_csv', 'ad_catalog'); # Connect to the users_csv data file

             # select from the datafile the id for the user ID from the array paased from the previous sub routine
             my $sql = "SELECT * FROM shiai_db WHERE shiai_id = ?";       # this is the SQL command we want to execute
             my $params = ($entered_shiai_id);                          # Theese are the parameteres we will use in the SQL command above
             print "$sql\n[$params]\n" if $DEBUG;                 # if we are in debug mode print the SQL statement
...
#             print p("result = ", @result)if $DEBUG;                # Prints the result of our SQL command if we are in debug mode.
             ...
             if (@result) {    #if the result array is in existence (ie we found the username) then...
                        ...
                        my $dbh = DBI->connect('dbi:AnyData(RaiseError=>1):'); # tell DBI we want to use the Anydata module in ./MyLibs
                        $dbh->func( 'shiai_db', 'CSV', 'data/shiai_csv', 'ad_catalog'); # Connect to the judoka_csv data file


                        # add the data into a new record
                        my $sql = "INSERT INTO shiai_db VALUES ( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,? )";       # this is the SQL command we want to execute, there SHOULD be 18 question marks
                        my @shiai_params = (@internal_shiai_data);                  # Make the parameters those passed to us from the previous routine (and originally from the user)
                        print "$sql\n[@shiai_params]\n" if $DEBUG;                 # if we are in debug mode print the SQL statement
...
sub create_shiai_db_files {
                      ...
                      if ( $shiai_type eq "ladder" && $flag eq "0") {
                       ...
                       if ($flag eq "0") {print p("Sorry, the shiai type was invalid and things went pear shaped!")};
...
sub create_ladder_data {
                      ...
                      # --------------------------------------------------------------------------------
                      my @the_shiai_data = @_;                     # This is the data transfered to us
                      my $shiai_ID = $the_shiai_data[1];           # A scaler to carry the id of this new shiai, used for nameing that data tables.
                      ...
                      # ------------------------------------------------------------------------------
                      my @ladder_fields = qw/ player_name ID position fights wins joined_date date_last_fight /;
                      my %ladder_field_def = (
                                        player_name => 'char(20)',
                                        ID => 'char(20)',
                                        position => 'char(20)',
                      ...
                      $dbh->do (
                               "CREATE TABLE ladder (" .
                                join(',', map { $_ . ' ' . $ladder_field_def{$_} } @ladder_fields) .
                                ")" )
                      ...
                      $dbh->do (
                               "CREATE TABLE history (" .
                                join(',', map { $_ . ' ' . $history_field_def{$_} } @history_fields) .
                                ")" )
                                or die "Can not create table";
                      ...
                      $dbh->do (
                               "CREATE TABLE challenge (" .
                                join(',', map { $_ . ' ' . $challenge_field_def{$_} } @challenge_fields) .
                                ")" )
                                or die "Can not create table";
...
print header(), start_html("e-Judo Test Area"), h1("CREATE NEW SHIAI"); # This line uses CGI.PM to to create the webpage
...
if ( param("id") ) {          # the ID parameter should be present when they first arrive and not when the form is filled in, so if it does exist we need to do the lines below
               print p("param:id exists, check the judoka limit against the number of judokas created for this user - START") if $DEBUG;

               my $user_id = param("id");                                # This line collects the user ID passed from the main menu identifying the user.
               my @user_data = get_user_data($user_id);                  # This line calls a sub routine which collects the user data from the user database
               print p("User Data = @user_data")if $DEBUG;               # Prints the result of our SQL command if we are in debug mode.
               my $shiai_limit = $user_data[12];                        # This collects the Judoka limit from the array
               print p("Shiai Limit = $shiai_limit")if $DEBUG;               # Prints the result of our SQL command if we are in debug mode.
               my $shiai_count = shiai_count($user_id);             # This line calls the subroutine that gets all the users Judoka Data

               if ( $shiai_count >= $shiai_limit)  {                  # If they have already created all the Judoka they are allowed return to main menu
                                                     print p("SORRY!!, Shiai Limit reached.");
                                                     print p("You can create a new Shiai");
                                                     print p("-> <a href=main_menu.cgi?id=$user_id>Click HERE to continue</a>");
                ...
                print_shiai_data_form($user_id);   # Call the sub routine which shows the user form to create a new Judoka
} else {
                my @temp_data = collect_shiai_data();
                my @temp_data2 = validate_shiai_data(@temp_data);     # Call the subroutine to validate the users input
                my @temp_data3 = generate_initial_values(@temp_data2);  # Create the initial values for the other fields not entered by users before adding to DB
                add_shiai_to_db(@temp_data3);         # Call the sub routine to add the Judo to the DB
