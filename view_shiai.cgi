...
sub read_shiai_data {
    ...
    my $shiai_id = $passed_info[0]
        ; # this is the shiai passed to us, we create this scaler just to make the script easier to understand
...
{ # the ID parameter should be present when they first arrive and not when the form is filled in, so if it does exist we need to do the lines below
    if ( param("shiai") )
    { # if the Shiai parameter exists then the user has selected a shiai to view
