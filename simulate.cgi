...
sub get_player_data {
    my @internal_data = @_;
    print $query->p( "data passed to GET_PLAYER_DATA = ", @internal_data )
        if $DEBUG;
...
sub fight {
    ...
    print $query->p( "Player1 Total Stats -> ", $player1_total_stats )
        if $DEBUG;
    ...
    print $query->p( "Player2 Total Stats -> ", $player2_total_stats )
        if $DEBUG;

    srand(time);
    my $rnd_fighter1 = int( rand( $player1_total_stats + 20 ) );
    my $rnd_fighter2 = int( rand( $player2_total_stats + 20 ) );
    print $query->p( "Player1 random -> ",
        $rnd_fighter1, " vs. ", $rnd_fighter2, " <- Player2 random" )
        if $DEBUG;
    my $result;
...
sub update_stats {
    ...
    my @internal_data = @_;
    print $query->p( "Data Passed to update_stats = ", @internal_data )
        if $DEBUG;
...
# select from the datafile the id for the user ID from the array paased from the previous sub routine
    ...
    print $query->p( "result = ", @winner_data )
        if $DEBUG
        ;    # Prints the result of our SQL command if we are in debug mode.
  ...
  # grab the number of wins and then add one to it and insert back into the DB
    ...
    print $query->p( "winner =", $winner, " and strength = ", $strength )
        if $DEBUG;
    ...
    print $query->p( "Loser =", $loser, " and strength = ", $strength )
        if $DEBUG;
...
sub remove_challenge {
    ...
    print $query->p("Challenge removed")
        if $DEBUG;    # just a reference in debug mode
