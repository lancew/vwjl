                   ...
                   # This sub rotine gets the Judoka the user has from judoka_csv file and returns the number
                    print p("START of get_judoka_data") if $DEBUG;
                      ...
                      my $entered_id = $internal_user_data[0];  # just so that it is clearer later we create a variable from the passed data
                       ...
                       # select from the datafile the id for the user ID from the array paased from the previous sub routine
                       my $sql = "SELECT * FROM judoka WHERE user_id = ?";       # this is the SQL command we want to execute
...
sub print_judoka_data_form {
    ...
    # The numbers indicate their position in the array(s)
                    ...
                    #     print p("Judoka ID: ", textfield("judoka_id"), " - This is the unique ID for this Judoka");
                         print p("Judoka Full Name: ", textfield("judoka_name"), " - This is your Judoka name (fullname)");
                         print p("Judoka Dojo: ", textfield("dojo"), " - This is the Dojo that your Judoka will belong to");
                         print p("Judoka Country: ", textfield("country"), " - This is which country your Judoka is from");
                         print p("Judoka Date of Birth: ", textfield("date_of_birth"), " - the date of birth (Fictional) for this Judoka.");
       	                 print p("Judoka Weight: ", textfield("weight"), " - the weight of your Judoka in Kilograms");
       	                 print p("Judoka Sensei: ", textfield("sensei"), " - the name of the Sensei (fictional) who teaches your Judoka");
       	                 print p("Judoka Biography: ", textfield("bio"), " - a brief biography of your Judoka, some backgrond info");
...
sub collect_judoka_data {
                    ...
                    #   $judoka_data[1] = param ("judoka_id");    # Now take the parameters from the completed form and add them to the array
                       $judoka_data[1] = "BLANK";    # Now take the parameters from the completed form and add them to the array
...
sub validate_judoka_data {
                      ...
                      print p("Start of validate_judoka_data") if $DEBUG;
                       my @internal_user_data = @_;                       # Take the array passed to this routine and assign it to an internal array
...
sub generate_initial_values {
 ...
 # user_id is already in the data array at this stage.
                      print p("Start of generate_initial_values") if $DEBUG;
                       my @internal_user_data = @_;                       # Take the array passed to this routine and assign it to an internal array
                        $internal_user_data[1] = $internal_user_data[0] . $internal_user_data[2];      # Make the unique ID for this Judoka the users ID plus the Judoka name.
                        ...
                        $internal_user_data[13] = "default";              # PASSWORD - set this to default (well at least for now)
                        $internal_user_data[14] = 0;                      # STRENGTH - set this to zero for now
                        my $loop;                                         # Initialise a variable to use for a loop
                        for ($loop = 15; $loop < 124; $loop++) {          # Loop from 15 to the last variable (there are 123)
                            $internal_user_data[$loop] = 0;               # This line sets all the remaining variables in the array to 0
                            }


                       print p("Validated data + initial values = ", @internal_user_data) if $DEBUG;
                       return (@internal_user_data);
...
sub add_judoka_to_db {
             ...
             my @internal_user_data = @_; # This line takes the user_data passed to us from the previous routine (@_) and allocates it to our internal user data array
             my $entered_judoka_id = $internal_user_data[1];  # just so that it is clearer later we create a variable from the passed data
             ...
             # select from the datafile the id for the user ID from the array paased from the previous sub routine
             my $sql = "SELECT * FROM judoka WHERE judoka_id = ?";       # this is the SQL command we want to execute
             my @params = ($entered_judoka_id);                          # Theese are the parameteres we will use in the SQL command above
             print "$sql\n[@params]\n" if $DEBUG;                 # if we are in debug mode print the SQL statement
             ...
             if (@result) {    #if the result array is in existence (ie we found the username) then...
                        ...
                        my $dbh = DBI->connect('dbi:AnyData(RaiseError=>1):'); # tell DBI we want to use the Anydata module in ./MyLibs
                        $dbh->func( 'judoka', 'CSV', 'data/judoka_csv', 'ad_catalog'); # Connect to the judoka_csv data file


                        # add the data into a new record
                        my $sql = "INSERT INTO judoka VALUES ( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,? )";       # this is the SQL command we want to execute, there SHOULD be 123 question marks
                        my @params = (@internal_user_data);                  # Make the parameters those passed to us from the previous routine (and originally from the user)
#                        print "$sql\n[@params]\n" if $DEBUG;                 # if we are in debug mode print the SQL statement
                        ...
                        print p("-> <a href=http://www.judocoach.com/e-judo/main_menu.cgi?id=$internal_user_data[0];>Click HERE to continue</a>");
             }
...
print header(), start_html("e-Judo Test Area"), h1("CREATE NEW JUDOKA"); # This line uses CGI.PM to to create the webpage
...
if ( param("id") ) {          # the ID parameter should be present when they first arrive and not when the form is filled in, so if it does exist we need to do the lines below
               ...
               my $judoka_limit = $user_data[11];                        # This collects the Judoka limit from the array
               print p("Judoka Limit = $judoka_limit")if $DEBUG;               # Prints the result of our SQL command if we are in debug mode.
               my $judoka_count = judoka_count($user_id);             # This line calls the subroutine that gets all the users Judoka Data

               if ( $judoka_count >= $judoka_limit)  {                  # If they have already created all the Judoka they are allowed return to main menu
                                                     print p("SORRY!!, Judoka Limit reached.");
                                                     print p("You can create no more Judoka");
                                                     print p("-> <a href=http://www.judocoach.com/e-judo/main_menu.cgi?id=$user_id>Click HERE to continue</a>");
                ...
                print_judoka_data_form($user_id);   # Call the sub routine which shows the user form to create a new Judoka
} else {
                my @temp_data = collect_judoka_data();
                my @temp_data2 = validate_judoka_data(@temp_data);     # Call the subroutine to validate the users input
                ...
                add_judoka_to_db(@temp_data3);         # Call the sub routine to add the Judo to the DB
