...
print p("Start show history") if $DEBUG;
      ...
      my $history_table = "data/shiai_data/" . $shiai[0] . "_hst";     # eg: data/shiai_data/userTest.ldr the ladder itself
      ...
      # ---------------------------------------------------------------
             ...
             $dbh->func( 'history_db', 'CSV', $history_table, 'ad_catalog'); # Connect to the users_csv data file

             # select from the datafile the id for the user ID from the array paased from the previous sub routine
             my $sql = "SELECT * FROM history_db";       # this is the SQL command we want to execute
...
#                                                               my $name = $sql_returned[0];          #create a scaler for tidiness which has the JudokaID
                                                               print ("<TR><TD>$sql_returned[0]</TD><TD>$sql_returned[1]</TD><TD>$sql_returned[2]</TD></TR>");
