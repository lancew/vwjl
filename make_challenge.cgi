...
my $max_post = 51_200
    ; #  To have messages appear use the following example in the code: print p("message in debug only") if $DEBUG;
...
# select from the datafile the id for the user ID from the array paased from the previous sub routine
    ...
    $dbh->disconnect()
        ; # we are done with the database for now, so disconnect from it (MAY NOT BE NECESSARY)
...
# select from the datafile the id for the user ID from the array paased from the previous sub routine
    ...
    {    # This while loop continues till there is no more reults
        ...
        if ( $found_id ne $judoka_id ) {
            ...
            print $query->br( $counter, " - ", $judoka_list[$counter] )
                if $DEBUG;
...
sub identify_available_judoka {
    ...
    print $query->p( "The number of entries in the passed list was :",
        $count )
        if $DEBUG;
    print $query->p( "The number of JUDOKA in the passed list was :",
        $number_of_judoka )
        if $DEBUG;
    ...
    print $query->p( "The JUDOKA_IDs in the passed list are :", @temp2 )
        if $DEBUG;

    # --------------------------------------

    my @available_list = @passed_list
        ;    #Just a cheat to make this work before the code goes in.

    return
        @available_list;   # Return the list of judoka available to challenge.
}
...
# Print a list of the available players to challenge with links to challenge them (calling this script again but with an extra parameter)
   ...
   # The list is in fact two lists, names the IDs. So divide the total by two.
    ...
    print $query->p( "The JUDOKA in the passed list are :", @judoka_names )
        if $DEBUG;

    my @judoka_ids = @incoming_list[ 0 .. $judoka_number - 1 ];
    print $query->p( "The JUDOKA_IDs in the passed list are :", @judoka_ids )
        if $DEBUG;
...
sub enter_chal_in_history {
    print $query->h4(
        "This is the sub that enters the challenge into the history")
        if $DEBUG;
...
if ( $query->param )
{ # If there is a parameter the user has already been to this page and enetered something so do something with their input
    print $query->h1("A parameter was passed so do Subs to do stuff")
        if $DEBUG;
    ...
    $shiai_id    = "lw1"
        ; # Setting this manually for now, needs to be a loop to access all Shiai at a later date.
    ...
    if ( $query->param("challenge") ) {
        ...
        print $query->p( "The judoka list returned = ",
            @returned_judoka_list )
            if $DEBUG;
        my @available_judoka
            = identify_available_judoka(@returned_judoka_list);
        print $query->p( "The availablejudoka list returned = ",
            @available_judoka )
            if $DEBUG;
...
{ # if there are no parameters then the user is here for the first time so show them the data entry form.
    print $query->h1("No Parameters passed so there was a problem");
    print $query->p("we pass the judoka_id & Judoka_name to this script")
        if $DEBUG;
