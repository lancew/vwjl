...
print header(), start_html("e-Judo Test Area"), h1("e-Judo test area"); # This line uses CGI.PM to to create the webpage
...
sub create_judoka_file {
                      ...
                      # ----------------------------------------------
                      my $judoka_table = "data/judoka_csv/";     # eg: data/shiai_data/userTest.ldr the ladder itself
...
harai_makikomi => 'char(20)'
                      ...
                      $dbh->do (
                               "CREATE TABLE judoka (" .
                                join(',', map { $_ . ' ' . $judoka_field_def{$_} } @judoka_fields) .
                                ")" )
...
sub create_users_file {
                      ...
                      # First create the array and hash to hold the table fields and data definitions
                      # ------------------------------------------------------------------------------
                      my @users_fields = qw/ id first_name surname date_of_birth email password active last_login create_date earnings cash judoka_limit sensei_limit dojo_limit team_limit rank /;
                      my %users_field_def = (
user_id => 'char(20)',
id => 'char(20)',
first_name => 'char(20)',
...
rank => 'char(20)'
                      ...
                      $dbh->do (
                               "CREATE TABLE users (" .
                                join(',', map { $_ . ' ' . $users_field_def{$_} } @users_fields) .
                                ")" )
                                or die "Can not create table";
...
sub create_shiai_file {
                      ...
                      # First create the array and hash to hold the table fields and data definitions
                      # ------------------------------------------------------------------------------
                      my @shiai_fields = qw/ owner_id shiai_id name closedate type eventdate admins_name active earnings cash entry_fee createdate first_prize second_prize third_prize fourth_prize fith_prize description /;
                      my %shiai_field_def = (
owner_id => 'char(20)',
shiai_id => 'char(20)',
name => 'char(20)',
...
description => 'char(20)'
                      ...
                      $dbh->do (
                               "CREATE TABLE shiai (" .
                                join(',', map { $_ . ' ' . $shiai_field_def{$_} } @shiai_fields) .
                                ")" )
                                or die "Can not create table";
...
# the following lines are excecuted if paramaters HAVE been entered
             ...
             my $name = param("ID"); # $name is the user name/ID from the webpage form enetered by the user
             my $password = param("ejudopass"); # $password is the password from the webpage form enetered by the user
             if ($name eq "" or $password eq "") {      # If the user does not enter any data then tell them they did wrong and stop
             ...
             my $sth = $dbh->prepare("SELECT * FROM users WHERE id = ?"); # select the password for the user entered anme from the database
             ...
             my $pass = $result[0]; # here we get a scaler variable from our array, as the is only one piece of data requested we choose array item 0
             if ($pass) { # if we get a match then...
                          if ($pass eq $password) { # if the password entered matches the one from the database then
                                     	print p("OK");
                                     	print (" <a href='main_menu.cgi?id=$name'>Click here to continue</a>"); # this line tells the user to continue to the main menu
...
# third, check if the shiai datafile exists, if not we will create it.
 if (-e "data/shiai_csv"){
       	...
       	print p("What is your user ID: ", textfield("ID"));        # which asks for a username & password
       	print p("Your Password: ", password_field("ejudopass"));
       	print submit(-name=>'submit button');
       	print end_form, hr;                                        # end the form
       	print ("else click <a href='create_user.cgi'>HERE</a> to create a new user"); # this line invites them to create a new user.
