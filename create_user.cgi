...
sub collect_user_input {
             ...
             my @user_data;  # initialise the array we will use to store all these pieces of data
             $user_data[0] = param("id"); # the next few lines alocate the info from the completed form to variables.
             ...
             $user_data[6] = "NO";   # ($active) the user is not immediately active, we will email them first.
             $user_data[7] = gmtime(); # ($last_login)set the last login time to now
             $user_data[8] = gmtime(); #($create_data) set the date we created this user to now as well
             $user_data[9] = 0; # ($earnings) They have not earned any thing yet so set it to Zero
             $user_data[10] = 50; # ($cash) This is their cash on hand, lets give them 50 credits by default
             $user_data[11] = 99; # ($judoka_limit) They can create 1 Judoka  ** Temporarily set to 99
...
sub add_user_input_to_db {
             ...
             my $sql = "SELECT id FROM users WHERE id = ?";       # this is the SQL command we want to execute
             ...
             if (@result) {    #if the result array is in existence (ie we found the username) then...
                        ...
                        my $sql = "INSERT INTO users VALUES ( ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,? )";       # this is the SQL command we want to execute
...
#                        print "$sql\n[@params]\n" if $DEBUG;                 # if we are in debug mode print the SQL statement
                        ...
                        print p("-> <a href=http://www.judocoach.com/e-judo/e-judo.cgi>Click HERE to continue</a>");
             }
...
sub print_user_data_form {
                         ...
                         print p("User ID: ", textfield("id"), " - This is the ID you will use to login to the system");        # what username do they want.
                         print p("First Name: ", textfield("first_name"), " - This is your REAL name");        # what is their real name
                         print p("Surname: ", textfield("surname"), " - This is your REAL last name");        # what is their real name
                         print p("Date of Birth: ", textfield("date_of_birth"), " - (DD/MM/YYYY) This is your date of birth, used if you ever need to prove who you are");
                         print p("Email address: ", textfield("email"), " - the email address you would like to use.");
       	                 print p("Password: ", password_field("ejudopass"), " - Choose a password to use on the system");
...
print header(), start_html("e-Judo Test Area"), h1("CREATE NEW USER"); # This line uses CGI.PM to to create the webpage
...
if (param()){  # If there is a parameter(or parameters) then validate, else show the login screen.
        ...
        } else { # if there are no parameters (the form has not yet been filled in then....
