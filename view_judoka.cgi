...
print p("Start display_judoka_data") if $DEBUG;
                         ...
                         # select from the datafile the id for the user ID from the array paased from the previous sub routine
                         my $sql_dataquery = "SELECT * FROM judoka";       # this is the SQL command we want to execute
...
my @judoka_found;                                                                # initialise the array where we shall collect the judoka information
...
# select from the datafile the judoka_id for the user ID from the array paased from the previous sub routine
             my $sql = "SELECT judoka_id,name FROM judoka WHERE user_id = ?";       # this is the SQL command we want to execute
             my @params = ($user_id);                          # Theese are the parameteres we will use in the SQL command above
             print "$sql\n[@params]\n" if $DEBUG;                 # if we are in debug mode print the SQL statement
             ...
             while (@sql_returned=$sth->fetchrow_array) {                       # This while loop continues till there is no more reults
                                                               ...
                                                               my $judoka_id = $sql_returned[0];          #create a scaler for tidiness which has the JudokaID
                                                               my $judoka_name = $sql_returned[1];        #create a scaler for tidiness that is the name of the Judoka
                                                               print p("<a href=view_judoka.cgi?id=$user_id&judoka=$judoka_id&name=$judoka_name>$judoka_name</a>");
...
print header(), start_html("e-Judo Test Area"), h1("VIEW JUDOKA"); # This line uses CGI.PM to to create the webpage
...
if ( param("id") ) {                                                            # the ID parameter should be present when they first arrive and not when the form is filled in, so if it does exist we need to do the lines below
  if ( param("judoka") ) {                                                      # if the JUDOKA parameter exists then the user has selected a Judoka so show this judoka
                        my $judoka = param("judoka");
                        my $judoka_name = param("name");
                        print p("Judoka_NAme: ", $judoka_name) if $DEBUG;
                        my @judoka_info = read_judoka_data($judoka);    # Call the subroutine to collect the judoka data from the database and assign it to an array
                        print ("The Judoka returned = ", @judoka_info) if $DEBUG;
                        print p(" <a href=entershiai.cgi?judoka_id=$judoka&judoka_name=$judoka_name>Enter a shiai</a>"); # Print a link to enter a shiai.
                        print p(" <a href=make_challenge.cgi?judoka_id=$judoka&judoka_name=$judoka_name>Make a challenge</a>"); # Print a link to challenge another judoka
                    ...
                    #    print p(" <a href=challenges.cgi?judoka_id=$judoka>Make/Accept Challenges</a>"); # Print a link to make challengesjust above the Judoka info.
                        display_judoka_data(@judoka_info);                      # Take the data collected and display it
                        ...
                        list_users_judoka( param("id"));                                      # This line calls the sub that lists the users Judoka (with links to view the selected Judoka)
