...
print p("Start show history") if $DEBUG;
      ...
      # ---------------------------------------------------------------
             ...
             $dbh->func( 'judoka_db', 'CSV', $judoka_table, 'ad_catalog'); # Connect to the users_csv data file

             # select from the datafile the id for the user ID from the array paased from the previous sub routine
             my $sql = "SELECT * FROM judoka_db";       # this is the SQL command we want to execute
...
print header(), start_html("e-Judo View Ranks"), h1("VIEW Ranks"); # This line uses CGI.PM to to create the webpage
