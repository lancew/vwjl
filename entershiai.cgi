...
my $judoka_name = $passed_info[1];                                                   # this is the judoka id passed to us, we create this scaler just to make the script easier to understand
print p("passed_info: ", @passed_info) if $DEBUG;
my @shiai_found;                                                                # initialise the array where we shall collect the shiai information
...
# Next we collect all the shiai IDs and names from the db
             ...
             while (@sql_returned=$sth->fetchrow_array) {                       # This while loop continues till there is no more reults
                                                               ...
                                                               # ----------------------------------------------------------------------------------------------
                                                              print p("<a href=entershiai.cgi?judoka_id=$judoka_id&shiai=$shiai_id&judoka_name=$judoka_name>$shiai_name</a>");
...
print p("Start enter_judoka_in_ladder") if $DEBUG;
      ...
      # ---------------------------------------------------------------
             ...
             my $sql = "SELECT id FROM ladder_db WHERE id = ?";       # this is the SQL command we want to execute
             ...
             if (@result) {    #if the result array is in existence (ie we found the username) then...
                        ...
                        # -------------------------------------------------
                        my $dbh = DBI->connect('dbi:AnyData(RaiseError=>1):'); # tell DBI we want to use the Anydata module in ./MyLibs
                        $dbh->func( 'ladder_db', 'CSV', $ladder_table, 'ad_catalog'); # Connect to the judoka_csv data file

                        # add the data into a new record
                        my $sql = "INSERT INTO ladder_db VALUES ( ?,?,?,?,?,?,? )";       # this is the SQL command we want to execute, there SHOULD be 18 question marks

                        my @ladder_params = ($player_name, $ID, $position, $fights, $wins, $joined_date, $date_last_fight);                  # Make the parameters those passed to us from the previous routine (and originally from the user)
                        print "$sql\n[@ladder_params]\n" if $DEBUG;                 # if we are in debug mode print the SQL statement
...
print header(), start_html("e-Judo Test Area"), h1("Enter Shiai"); # This line uses CGI.PM to to create the webpage
...
if ( param("judoka_id") ) {                                                            # the judoka-Id is passed to us from the previous script, so should exist. If it does not something is wrong so exit
  if ( param("shiai") ) {                                                      # if the Shiai parameter exists then the user has selected a shiai to enter
...
#
        ...
        list_shiai( @data_to_pass );                                       # This line calls the sub that lists the shiai, we pass the judoka ID so that we can add it to the HTTP link
        }                                                                       # End of the if statement related to the Judoka parameter
} else {                                                                        # If the judoka_id parameter does not exist they should not be here so display a link to the front page (e-judo.cgi)
        print p("Problem!");
